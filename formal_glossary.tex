...
\fix{mr}{it needs to be stressed that without the formalization of
the concept of error, we cannot properly formalize the concept of weakness
which remains just a region as any other region with the only 
constraint that it must be connected to another region called Vulnerability (which 
in turn depend on the definition of error)}

\fix{mr}{the following definition should be moved (and detailed)
to section~\ref{engineering} when the concept of weakness is defined}
...
\fix{mr}{shouldn't we consider the agent who introduces the weakness as dishonest? don't we say this in part 1?}
\end{example}
...
\fix{mr}{similar to weakness and vuln, the following definition should be kept here just to show that
it is impossible to defined those concept without the proper definition of the concept of error. and the definition should
be introduced in section~\ref{engineering} when the concept of weakness is defined}

...
We now conclude this section by detailign the line of reasoning we want to
apply to our investigation.
The epistemologic concept of Knowledge can be equated to the
concept of Truth.  Anything that is known is, therefore, also true. It is also
believed that Knowledge, defined as we did, is supposed to be inapprehensible.
In contrast, Beliefs are potentially true and leads new hypothesis. Due to the
structure of deductive reasoning we can only start from hypothesis and test the
deductive conclusions on the reality. Whatever a scientific-security-theory is,
it must be defined starting from hypothesis.  A sound reasoning must lead to
conclusion that shall be tested on a real system. Thefore, only at the end of
the reasoning we will be able to test, and judge, the quality of the theory
itself. The identification of hypothesis should start from probably true
beliefs (which means from Information), and then from the state of the art.
Instead of having a ``related work'' section, we review and cite the literature

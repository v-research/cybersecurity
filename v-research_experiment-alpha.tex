...
divided into the following phases (explained in the remainder of this sub-Section):
...
\Paragraph{Exploitation Flow} An \emph{Exploit} is an \emph{implementation} of
a Vulnerability, where a Vulnerability is one of the many ways to transform a
Weakness into a malicious process that can be applied\footnote{Meaning that the
...
\begin{enumerate}
	...
	\item CAPEC provides a \emph{dictionary} of \emph{known} attack patterns.
		As stated largely in Section~\ref{sec:}, this dictionary contains
...
\begin{enumerate}[noitemsep]
\setcounter{enumi}{4}
	\item an agent (Mankind\footnote{For the sake of readability we focus only on Mankind as an agent}) the exploitation flow can be used to hack a system. We use the term hack (as hacking) to stress that nothing prevents this process from being honest, however, for the sake of simplicity, we focus on Attacks which are malicious by definition.
	\item An Attack is applied to a system with malicious intent by an agent
	\item The application of the attack results in an Incident which impacts, i.e. poses a Threat to, an Asset
\end{enumerate}

\Paragraph{Hacking Flow}


\begin{example}
given in natural language in the description of the 
CWE-116 which we can analyze as follows:
\begin{itemize}
	\item ``the software prepares a structured message''
	\item the message is intended ``for communication with another component''
	\item ``but''
	\item ``encoding or escaping of the data is either missing or done incorrectly''
	\item ``[\ldots] result, the intended structure of the message is not preserved''
\end{itemize}
\end{example}

In this Vulnerability is seen as a process through which an agent (i.e. Mankind)

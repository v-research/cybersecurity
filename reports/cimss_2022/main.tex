...
\fix{mr}{
\begin{enumerate}
  \item Herley's work (1 paragraph)
  \item Implications on Risk Assessment (no theory implies no complete set of weakness which, in turn, implies no risk assessment)
  \item 1 page: contributions
\end{enumerate}
}
...
%%as in \autocite{hintikka2013inquiry}, we can
...
%%(where we ask if
%if we detail the question as:
%there exists a predicate $P$ such that, given any arbitrary system $s$, when
%$P$ holds for $s$ then $s$ can be considered secure?
%%), 
%When the answer is (for
%example) ``if the system is confidential, the system is secure'' the reasoning %formula
...
attacker has a set of hard-coded inference rules that defines how the attacker
...
\begin{itemize}
	\item In \autocite{Hintikka1962knowledge}, Hintikka describes the
		concept of belief (as epistemological
...
\emph{beliefs} (internal information considered true by the agent), and
\emph{assertions} (an exchange of information between agents). % it provides.
...
%As in \autocite{Steup2020epistemology}, 
%the only objective of information is to
%exchange beliefs between agents and, 
As defined in \autocite{Hintikka1993Information},
...
beliefs to define the actual behavior of an agent or a system; meaning
that an agent has its own beliefs on top of which it bases its behavior,
the consequences of which, in turn, will update its initial beliefs.
%; i.e. we consider
...
\begin{enumerate}
	...
	\item We consider a set of axiomatic \emph{facts} determining how the
		system shall be. Similarly, requirements determine which facts
		must hold when the system is implemented\footnote{We consider facts as
		definitory rules\autocite{Hintikka1993Information} since they
		don't define how a real system is finally implemented but how
		it shall be, trough a series of requirements.  Those
		requirements may not hold, through insecurity, in the
		implemented system.}. Specifically, facts/requirements describe:
		the functional architecture of each agent, and the
		physical/structural (HW/SW) architecture of each subsystem of
		agents and agent within a subsystem.
\end{enumerate}
...
discrete sets) such as the one provided in \autocite{NIST2020NVD,MITRE2020CVE}
...
\begin{figure*}
	\centering
	\includegraphics[width=.7\textwidth]{results.pdf}

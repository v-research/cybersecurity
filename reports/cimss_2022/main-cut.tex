...
\paragraph{Context and motivation.}\ON{General: let us remind ourselves to address (at least discuss between us) the concerns of the reviewers.}
...
formulation of a system in a framework called $\abftheory$-framework\ON{Qui, direi o "framework called ABF" o "framework called the ABF-framework"}, and
predicts 4 main classes of weaknesses. Those classes of weaknesses\ON{rimuoverei ripetizione di "of weaknesses"} are used to
...
\begin{enumerate}
	\item \emph{System Specification}: the \emph{functional and physical requirements} \ON{proporrei di togliere l'and dal corsivo} are defined.
	\item \emph{Architecture Design}: the specification is structured into \emph{functional and physical architectures}\ON{same as the above}.
...
As in \autocite{Santaca2016abf}, we define agents\ON{so far, we spoke about systems, from now we focus on agents. Shall we briefly motivate why we think of systems in term of agents? Or, at least, say that we use the two terms intercambiabilmente? Later on in the paper, we do say "agent (i.e. system)", but that comes much after this, and so the text inbetween might be puzzling to some.}\ON{Further note: In section 4.1 we explicitly state this. Perhaps shall we here just say "we consider a CPS as a hierarchy of agents, as we will detail later in Section 4.1"? (or something similar)} as a meronomy (an
...
\begin{definition}{\bf System State --}\label{def:opsystem}
	A CPS system (or a sub-system) state is defined as the\ON{the->a} tuple
	...
	assertions, beliefs (i.e. the beliefs generated by the behavior)\ON{is "generated" the right word here? Anyhow, the bracketed part is not super clear to me.}, and
	facts respectively, expressed as requirements.
\end{definition}

\ON{Shouldn't we introduce these terms here? A, B, F? I mean even with short sentences: assertions are the exchanged data, facts requirements, beliefs are the behaviour...}

As in \autocite{Santaca2016abf}, it follows that, by defining
a system with \ON{with->as?} a fixed number of regions, there exists
...
For completeness, we report in Appendix~\ref{app:agcalc}\ON{here}
...
implement or diverge from the intended nominal behavior.\ON{Here, I would explicitly say that what we mean is that, only when the assertiosn, beliefs and facts are equal, then the system is behaving as excepted. That is, I would explicitly say is that the only nominal state is EQ,EQ,EQ. I mean, it is in the table, but a bit implicit to me.} We note 
...
agent (i.e. a system) exist w.r.t. the $\abftheory$-framework (i.e.\ON{e.g.?} $54/125$ in RCC5), a
...
regions (i.e.  $\abf$), and then categorize different
agents as tuples of the three regions.\ON{but we don't actually do this, right? Maybe I missed it. If not, we should remove this sentence. If yes, then we should clarify this. (At least to me :-))}  For the sake of simplicity, soundness is
opposed to non-soundness in the following, however, with the RCC, one should\ON{should->could?}
...
between two agents.\ON{first time we say this!} An agent has two main categories of assertions,
...
of notation,\ON{alternative: in the text we drop the $\Phi$ when the content of the assertions is not relevant.} in the text we simply write $\assertionRegion_{s\rightarrow a}$ when we abstract
...
\begin{itemize}
	\item \Rcc{$\assertionRegion_{s\rightarrow a}$}{$\beliefRegion$},
		...
		specification is complete.\ON{Do we perhaps mean "sound"? We used sound above}
...
$\Rcc{\rassert{r}{*}}{\beliefRegion_a}$, for output-port (where $*$ is a wildcard for an agent)\ON{should this $r$ be an $a$?}.  A port is, in
...
An input port is, in the $\abftheory$-framework, defined secure as long as the relation
	...
	as $A=\emptyset$).\ON{Do we use the weight/$|A|$ thing in the proof? I don't see it... If not, should we get rid of it?}
	\begin{enumerate}
...
\begin{definition}{\bf Mono-directional Channel (bottom-channel) --}\label{def:monochannel}
	...
	defined as\ON{remove defined as} an agent whose behavior is (dogmatically) defined as: to
...
theoretical configurations ($7^2$ because there are $6$ insecure types of port,\ON{refer to Theorem 1?}

...
\begin{abstract}
	...
	channels.  Our theory allows the definition of the security of a system
	as a mathematical formula correlating the security and insecurity
	configurations of a system.  We implemented a prototype cybersecurity
...
be considered correct as long as no empirical experiment falsify it. For
...
insecurities of a system (which, by negation would become a theory on
...
Our hypothesis is that cybersecurity attacks are caused by 
...
security of a system. With our approach, a list of weaknesses emerges
...
the tool-assisted security risk assessment of a CPS (with an ad-hoc example,
...
\fixnote{Luca}{I think that this section needs thorough rewriting. First of all, you are trying to say too many things, many of which are not really relevant for what you want to say. Second, you are not just reviewing but also introducing some new things that are part of your contributions and this is quite confusing. Third, there are some claims that, in my view, are wrong or at best misleading. Shorten, simplify, aiming to distill really only what is relevant. As it is now, the text is quite confused and confusing.}
\begin{figure*}[t]
	\centering
...
in turn, between safety and cybersecurity\fixnote{Luca}{You spoke of security before, which is much wieder than cybersecurity. This is something you have to be very careful with... political scientists would correctly claim that security is much more than just cyber and that their term is the correct one to contrappose to safety}) is believed to be on the different
...
be malicious even if it is due to the lack of skills.}.\fixnote{Luca}{Secondo me tutta questa prima discussione \`e alquanto inutile e si pu\`o riassumere in un'unica frase.}
...
and thus unpredictable. Currently, our understanding of cybersecurity is
...
and attacks (e.g. CAPEC~\autocite{MITRE2020CAPEC}, ATT\&CK~\autocite{MITRE2020ATTACK}).\fixnote{Luca}{Come detto, sono fortemente in disaccordo con questa frase. Parte del nostro understanding \`e l\`i, ma non tutto. Tutto il lavoro teorica e formale non \`e contenuto l\`i. Mi sembra tra l'altro un'affermazione alquanto inutilmente provocatoria} Systems are
nowadays tested against 
known attacks, ignoring that considering a system secure
if no known attack is possible is against the scientific method.\fixnote{Luca}{No, non \`e una scelta ma \`e semplicemente ignoranza di chi fa i test. Chi fa i test seriamente questo lo sa benissimo. E non solo perch\'e \`e contro lo scientific methos ma anche perch\'e testing it not proving}
As an example, if we have a system which is secure against all of the
vulnerabilities in the CVE we often just need to wait for a month to have 1000+ more
vulnerabilities to test \autocite{NVD2020dashboard}. If even just one of those new vulnerabilities
affects our system, that falsifies the hypothesis that a
system is secure if no known attacks apply.\fixnote{Luca}{But, again, the hypothesis was utterly wrong from the start, even without considering the scientific method. No person who is really knowledgeable of testing would ever claim that testing shows the absence of all attacks. Maybe this is what is done in industry, but then industry is simply wrong (it would not be the first time) and people in academia have been saying so for ages. It is not you argueing it.. I believe that building the argument as you do is wrong. I would suggest: (1) Industry uses testing believing that it is enough. (2) It is well know that testing cannot show the absence of attacks. (3) The scientific method makes this even more evident and in fact we show ...} 
...
into the opposite direction of the ``implement and test'' approach.\fixnote{Luca}{Questa ultima frase \`e appesa qui senza una vera chiara connessione a quanto detto finora.}
...
(therefore should be considered hypotheses and not definitions).\fixnote{Luca}{Hypotheses and definitions don't type check. Moreover, many standards require formal methods proofs, don't they?} While
...
impossibility of defining a theory that correctly predicts failures.\fixnote{Luca}{This sentence does not follow grammatically}  A failure
...
the terms that structure our current understanding of security.\fixnote{Luca}{I disagree. It should be cybersecurity, not security. Moreover, as I mentioned, the figure is misleading when it talks about blac-hat hacking. I also don't like ``Manking'', as mankind does many of the things, not just designing systems} 
\begin{itemize}
...
\autocite{wiki-mankind}, while the concept of \emph{Nature} is
		...
		\begin{itemize}
			\item There exist several terms to refer to an
				...
				equivalent.\fixnote{Luca}{Grammar issues}  From now on, we consider the
...
\begin{itemize}
	\item \emph{Weakness}. The definition given by the MITRE
		...
		vulnerabilities.\fixnote{Luca}{Worth to say that many security issues are not software weakenesses, e.g. cryptography issues or the way cryptography is used. Moreover, in cyber-physical systems, security issues might be due to hardware issues. Again, I think that you are trying to say too many things. Focus on what you really want to say.} A software vulnerability, such as those
...
\begin{itemize}
	...
	\item An \emph{Exploit}\footnote{We note that the term exploit is only
		used as a verb in~\autocite{ISO2009information}}\fixnote{Luca}{So what? Is it The use in~\autocite{ISO2009information} is incomplete? } ``[\ldots]
...
called adversary or attacker).\fixnote{Luca}{This is slightly misleading, as you have not discussed this above. You mean in the review that you did you discovered that...} This, however, just moves the problem of
defining what cybersecurity is as the problem of defining what a dishonest
agent can or cannot do in a system, where an agent is any virtual or
...
evident.\fixnote{Luca}{Which hypothesis?} For example, a software can be considered an agent of the system and
...
(to name a few).\fixnote{Luca}{I foudn this sentence very hard to follow. Moreover, I would be careful to speak of ``predefined actions'' as basically the DY can do anything so it is not cleaer what you are referring to. Of course, you could this that this excludes, e.g., human errors (say as in the work I did with Diego) or more, but then you should clarify.} As depicted in Figure~\ref{fig:causality}, in order to define
...
in the system.\fixnote{``cause-by'' is grammatically wrong, but more importantly the presence of a vulnerability does not necessarily cause an attack, but allows it. Moreover, you speak of Patterns for the first time.} Those Vulnerabilities are, in turn, cause-by the presence of
...
$\abf$-framework  defined in \autocite{Santaca2016abf} and, in
...
has to ports denoted by ``I'' (with incoming assertions) and ``O'' (with outgoing assertions).
...
\begin{figure}[t]
	...
	\caption{xample of system and agent structure}
...
CVSS\autocite{Mell2007CVSS}) to assign a quantitative evaluation 
...
the reflexive, antisymmetric, and transitive \emph{Parthood} relation $\subseteq$.  
...
loss of generality. Using RCC5 instead of RCC8 prevents us from 
considering tangential connections between spatial
Regions. However, tangential connections in RCC8 can be considered as 
special cases of the more general spatial relations considered in RCC5.

In Table~\ref{tab:rcc}, we
...
where $\rcc(X,Y)$ on two generic regions $X,Y$ represents one of the possible RCC relations
between $X$ and $Y$.
...
In Figure~\ref{fig:soundness},
the relation between facts, and
assertions and beliefs (as inputs and outputs of the Behavior of an agent)
is illustrated. assertions and beliefs generated by the design of a system
may not be exactly aligned with what the facts mandate (i.e. what the specification mandates.
The relation between facts, and assertions and beliefs 
can be used as a metric to determine the soundess of the 
design with respect to the specification.

We categorize agents by first
...
of non-soundness\fixnote{mv}{In particolare, spiegherei meglio Fig. 6 e il discorso della soundness}. 
For example, in RCC5, if we consider EQ between two Regions
...
the $\abf$-framework) and, based on that definition, what the security risk is and
...
\begin{figure}[t]
	...
	\label{fig:ioports}
\end{figure}
Since the $\abf$-framework is a theory of agents, we could consider ports as
...
\begin{definition}{\bf Secure Input Port --}\label{def:secport}
	A secure input port always allow information as
...
\begin{proof}
An input port is, in the $\abf$-framework, defined secure as long as the relation
...
ports, since the $\abf$-framework is a theory of agents.  In fact, if a channel
...
$\abf$-framework to channels (the analysis of the relation
...
ports.  Io order to define a functional block
...
four main categories of weakness: Replace, Insertion, Drop, Injection. We can,
...
\appendix
%\section{Engineering $\abftheory$-framework}

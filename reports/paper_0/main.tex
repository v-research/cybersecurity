...
A scientific theory is an explanation of a phenomenon such that the explanation
follows the scientific method. The scientific method is an \emph{empirical}
method that aims at mitigating potential fallacies in theories.  K. Popper
famously argued (e.g. in \autocite{Popper1959logic}) that a scientific theory
can never been verified but only falsified, that a theory should not be
conceived by using the principle of induction\footnote{A.  Einstein wrote to K.
...
theory. If we consider a natural (real-world) phenomenon, such as gravity, we
can think of a scientific theory as an \emph{abstract} mathematical formula (or
a set of formulas) that axiomatically define a certain (natural) phenomenon in
...
phenomenon it describes. Similarly, the results of the application of a
...
characterize the phenomenon it describes? With his famous ``demarcation
principle'', K. Popper draw a line between scientific and pseudo-scientific
...
of gravity, in its mathematical setting, can predict how object attracts with
...
insecurity of a system (which, by negation it would become a theory on security). 
...
security properties verified).  For example, in the protocol verification
...
(e.g. ) used for the engineering of systems starts with the requirements
...
that may generate security vulnerabilities later in the development process. An
infamous example is the KRAK attack on the WPA2 4-way handshake protocol where
the authors wrote\autocite{krak2020web}: ``our attacks do not violate the security properties proven
in formal analysis of the 4-way handshake'' but ``the problem is that the
proofs do not model key installation.''.

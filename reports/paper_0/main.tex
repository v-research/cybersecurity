...
Instead of reasoning of what makes a system secure or insecure,
...
to predict security weaknesses in a system\fixnote{mv}{Se capisco bene, questa intro ha di nuovo un valore di motivation, in particolare mostrando i problemi degli approcci correnti; mi aspetterei un paio di frasi in più alla fine per spiegare perché il vostro approccio risolva il problema}.
...
between the outside of the port to the inside of itself \fixnote{mv}{non chiaro, riformulare}.
...
$\Rcc{\rassert{r}{*}}{\beliefRegion_a}$ for the output-port. 
A port is, in fact, syntactic sugar to express the 
...
Given that a mono-directional bottom-channel is assumed\fixnote{mv}{C'è un motivo per cui fate l'assunzione che il canale sia sicuro mentre non lo facevate per le port?} to be perfectly
...
output port; therefore there exist $(7^2)-2=47$ theoretical configurations
...
$\Rcc{\assertionRegion_2}{\beliefRegion_3}$)\fixnote{mv}{Come dicevo in un'altra nota, userei sempre la freccia nei pedici delle asserzioni (oppure mai, ma insomma in maniera uniforme). Inoltre l'ordine degli argomenti della relazione rcc è rilevante? O rcc è simmetrica? In ogni caso, da qualche parte lo direi}.  The same result can be obtained
...
described afterwards\fixnote{mv}{Questo è difficile da seguire. Non si può spostare a dopo aver definito i functional blocks e il modo in cui li interpretate nel vostro MAS?}.  As depicted in Figure~\ref{fig:atom}, so far we have
...
then channels are atomic (otherwise raising infinite regress), we can only
...
\begin{enumerate}[start=50, label={W\arabic*)}]
	\item $\po{\beliefRegion}{\factRegion}$\fixnote{mv}{Dalla def. sopra un functional block ha un input e un output belief. Capisco che ora entrino in gioco i fact ma non capisco perché qui nelle weakness consideriate solo una belief (ed è quella di input o di output?). Oppure qui B sta per behavior? In ogni caso, chiarirei} the component has a Byzantine behavior where occasionally outputs the expected
...
and architecture, there may exist a factorial\fixnote{mv}{chiarire} requirement (fact) predicating over them.
...
\begin{itemize}
	\item $\eq{\beliefRegion}{\factRegion}=\eq{\beliefRegion_3}{\beliefRegion_1+\beliefRegion_2}$, 
		\fixnote{mv}{Is this the sum of two regions? How is it defined? If it's just the name of the region corresponding to the facts, I would use a different name: I find this confusing} where $\beliefRegion_3$ represents the region of all the
		outputs\fixnote{mv}{Non mi è chiarissimo cosa intendiate con ``all the outputs''} of the functional block $sum$ while the region
	...
	\item $\ppi{\beliefRegion}{\factRegion}=\ppi{\beliefRegion_3}{\beliefRegion_1+\beliefRegion_2}$, there exist some outputs that are not the result of the summation of the inputs but given the correct inputs the function always outputs correctly.
	\item
...
\fixnote{mv}{Questo paragrafo mi fa un po' paura :) Non capisco del tutto da dove venga la formula. Per ogni agente: in r(n,k) ho tutte le possibili configurazioni e da queste poi sottraggo quelle che non si possono verificare? Ma tra tutte le possibili non ce ne sono anche di buone? Ad esempio quelle con EQ non sono in genere buone? Il risultato è compreso tra 0 e 1 o può essere anche negativo?}

...
An input port is, in the $\abf$-theory, defined secure as long as the relation
	...
	the DR relations, they have no element (subregion) in common. Lets
	define a function weight $|X|$ such as, for any region $X$, it
	represents the smallest possible cardinality of a topological base for
	$X$; where a base is a collection of regions in a topology such that
	...
	$|\assertionRegion|\neq|\beliefRegion|$. If, at the end of a
	communication through the input-port
	$|\assertionRegion|\neq|\beliefRegion|$, the two regions have a
	different number of subregions and then there exist only two mutually
	exclusive options: 
	\begin{itemize}
		\item either $|\assertionRegion|>|\beliefRegion|$, where one
			ore more assertions didn't become Beliefs, which we
			call \emph{drop port} since its behavior drops and
		...
		\item or $|\assertionRegion|<|\beliefRegion|$, where there are
			...
			incoming communication, which we call \emph{insertion
			port} since its behavior generates new Beliefs
			unrelated to the incoming communications. 
	\end{itemize}
	If $|\assertionRegion|=|\beliefRegion|$, given that information is monotone by nature (due to its entropy)
there exist only two mutually
	exclusive options: 
	\begin{itemize}
		\item either information has been generated and transferred, increasing the cardinality of both
			...
			means that Assertions has reached the port and new
			Belief have been generated by the port but no
			correlation the elements of the two regions exist, and
			we call it \emph{injection port} since it has modified
	...
	Similarly, 
	\begin{itemize}
		\item given that $\dr{\assertionRegion}{\beliefRegion}$ is
			$\dr{\assertionRegion}{\beliefRegion}$ except for the
			intersection in which case is
			$\eq{\assertionRegion}{\beliefRegion}$ (see
			\autocite{Santaca2016abf}),
			we call it a selective injection port ($s\selectivedropport a$),
		\item if $\pp{\assertionRegion}{\beliefRegion}$, we have that
			some Belief are not correlated to incoming assertion
			and we call it ($s\selectiveinsertport a$)
		\item if $\ppi{\assertionRegion}{\beliefRegion}$ we have that
			some incoming Assertions are not correlated to Belief 
			and we call it ($s\selectiveinjectionport a$)
	\end{itemize}
\end{proof}

...
specificaiton to implementation, in this text we only consider the process from specificaiton to design.}
...
\begin{itemize}
	...
	\begin{enumerate}[start=49, label={W\arabic*)}]
		\item $\dr{\beliefRegion_O}{\factRegion_O}$, not all the specified
			outputs are produced
		\item $\po{\beliefRegion_O}{\factRegion_O}$ , only some but not all of the specified
			outputs are produced along with some non-specified output are produced
...
when the property of availability is expressed over
	...
	or drop channels. 
\end{definition}\fixnote{mr}{all security definitions defines when the property is
not preserved instead of when it's preserved}
...
\begin{hypothesis}{\bf System Security Design --}\label{hyp:security}
	A system security design is given by a system security specification, i.e. a set of
	requirements on the integrity and availability over the physical and
	functional architectures, that uniquely defines the designs built on
...
\begin{hypothesis}{\bf System Insecurity Design --}\label{hyp:insecurity}
	...
	requirements on the physical and functional architectures, there exist
